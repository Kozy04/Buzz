...
async function loadData() {
  ...
  const dSmtpPass = document.getElementById("settingsDispatchSmtpPass");
  if (dSmtpPass) dSmtpPass.value = settings.dispatchSmtpPass || "";

  const dSmtpPort = document.getElementById("settingsDispatchSmtpPort");
  if (dSmtpPort) dSmtpPort.value = settings.dispatchSmtpPort || "465";
...
function saveSettings() {
  ...
  settings.dispatchSmtpPass = document.getElementById("settingsDispatchSmtpPass")?.value.trim() || "";
  settings.dispatchSmtpPort = document.getElementById("settingsDispatchSmtpPort")?.value || "465";
...
function updateDispatchGuideUI() {
  ...
  const smtpGroup = document.getElementById("cpanelSmtpCredentialsGroup");
  if (smtpGroup) {
    smtpGroup.style.display = (engine === "cpanel") ? "flex" : "none";
  }
}
...
async function sendEmailDirectly({ to, subject, body, attachmentBase64, attachmentName }) {
  ...
  const payload = {
    ...
    smtpPass: settings.dispatchSmtpPass?.trim() || "",
    smtpPort: settings.dispatchSmtpPort || 465,
    smtpUser: settings.dispatchFromEmail?.trim() || "",
